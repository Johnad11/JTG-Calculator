...
const Performance = ({ trades, globalBalance, updateGlobalBalance }) => {
    ...
    const stats = useMemo(() => {
        ...
        let growthPct = 0;
        if (globalBalance) {
            currentBalance = parseFloat(globalBalance) + netPnL;
            growthPct = ((currentBalance - parseFloat(globalBalance)) / parseFloat(globalBalance)) * 100;
        }

        return { total, wins, loss: total - wins, winRate: total > 0 ? ((wins / total) * 100).toFixed(1) : 0, netPnL: netPnL.toFixed(2), profitFactor: grossLoss > 0 ? (grossProfit / grossLoss).toFixed(2) : (grossProfit > 0 ? 'MAX' : '0.00'), bestPair, currentBalance: currentBalance.toFixed(2), growthPct: growthPct.toFixed(2) };
    }, [trades, globalBalance]);

    const saveBalance = () => {
        updateGlobalBalance(tempBalance);
        setIsEditing(false);
    };
                ...
                <div className="bg-jtg-card border border-jtg-blue/30 rounded-2xl p-6 mb-8 shadow-xl relative overflow-hidden">
                    ...
                    {!globalBalance || isEditing ? (
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <div><p className="text-[10px] text-slate-500 uppercase font-bold">Starting</p><p className="text-sm md:text-lg font-mono text-slate-300">${parseFloat(globalBalance).toLocaleString()}</p></div>
                            <div><p className="text-[10px] text-slate-500 uppercase font-bold">Current</p><p className="text-lg md:text-2xl font-mono text-white font-bold">${parseFloat(stats.currentBalance).toLocaleString()}</p></div>
